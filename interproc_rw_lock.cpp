...
namespace tepp
{
	...
	interproc_rw_lock::interproc_rw_lock(HANDLE w, std::vector<HANDLE> && r) noexcept(true)
		: _w(w)
		, _r(std::move(r))
	{
	}

	interproc_rw_lock* interproc_rw_lock::try_create_interproc_rw_lock(const std::string & name, int max_readers) noexcept(true)
	{
		if (max_readers >= 64)
			return nullptr;
	...
	bool interproc_rw_lock::read_under_lock(action0 f, std::atomic_bool * c, ulong_t pr_wait_ms, ulong_t pw_wait_ms , std::string * err_msg ) noexcept(true)
	{
	...
	bool interproc_rw_lock::write_under_lock(action0 f, std::atomic_bool * c, ulong_t pr_wait_ms, ulong_t pw_wait_ms, std::string * err_msg) noexcept(true)
	{
		if (!is_ok(try_get_lock_infinite(_w, pw_wait_ms, c, err_msg)))
			return false;
